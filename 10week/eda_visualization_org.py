...
def load_data(file_name):
    ...
    # 공격 여부 판단 (label 컬럼의 값이 'Anomalous'이면 1, 아니면 0)
    df["is_attack"] = (df["label"] == "Anomalous").astype(int)
    
    return df

# --- [교수님 가이드] 사이드바 데이터셋 선택 메뉴 ---
st.sidebar.header("📊 분석 데이터 선택")
...
# 선택에 따라 다른 파일 로드
if selected_dataset == "1만 건":
    df = load_data("csic2010_requests.csv")
else:
    df = load_data("csic2010_requests_org.csv")
# 사이드바: 기본 정보
